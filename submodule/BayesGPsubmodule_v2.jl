...
function updateyprd_m(ytend,ydel,lp,R,sigR)
    c_yprd = [ 0.0 for i=1:lp]
    rtmp = rand(Normal(R,0.8*sigR))    
    for i = 1:lp
        dif = ydel * rtmp * (rtmp^(i)-1)/(rtmp-1)
        c_yprd[i]= ytend - rand(Normal(dif,0.01*abs(dif)/(i^2))) 
    end
    #println("c_yprd $c_yprd")
    return c_yprd
end
...
function main(xtrain,ytrain,yun,mstep,numN,Mysigma,muy,mstd)
    ...
    tstep = 1
    NDist=Normal(R,0.8*sigR)
    for ith=1:numN 
        tTheta,typrd,tyd,tyd2,tPH,tlogprior,tllh,tlogpost,tder,tder2,tmujoint,tSjoint=Pv[ith]
        tyd=0*xd; tyd2=0*xd2
        Accept,tPH,tlogprior,tllh,tlogpost,tder,tder2,tmujoint,tSjoint = evalPH_t(Kernel,Ktt,Kpt,Kpp,tstep,mstep,ith,xtrain,ytrain,lt,Mysigma,muy,mstd,xprd,yprd,lp,xd,yd,ld,xd2,yd2,ld2,iThetas[ith],Monotonic,Convex,"Sample",false,1.e+10)
        rtmp = rand(NDist)
    ...
    for tstep=2:mstep
        ...
        @time for ith=1:numN 
            ...
            tmp=evalPH_t(Kernel,Ktt,Kpt,Kpp,tstep,mstep,ith,xtrain,ytrain,lt,Mysigma,muy,mstd,xprd,typrd,lp,xd,tyd,ld,xd2,tyd2,ld2,c_Theta,Monotonic,Convex,"Sample",false,prePH)
            ...
            n_der,n_der2 = eval_der(tstep,mstep,xtrain,ytrain,xprd,c_yprd,mstd,R,sigR,sigfac)
...
function Simple_inv(tmpA::Array{Float64,2}) 
...
function calcSj(cLinv::LowerTriangular{Float64,Array{Float64,2}},Kpt::Array{Float64,2},Kpp::Array{Float64,2},yt::Array{Float64,1})
    tmp=cLinv*transpose(Kpt)
    Kpt*(transpose(cLinv)*(cLinv*yt)) , Kpp - transpose(tmp)*tmp
end
...
function eval_der(tstep,mstep,xt::Array{Float64,1},yt::Array{Float64,1},xprd::Array{Float64,1},yprd::Array{Float64,1},mstd,R,sigR,sigfac)
    der = 0.0 ; der2 = 0.0
    nu  = nu_t(tstep,mstep)
    ...
    if xprd[1]> 0 
        ...
        if lp > 1 
            ...
            if lp>2 
                for k =1:lp-2
                    if abs((yprd[k+1]-yprd[k+2])*mstd) < 2.e-4 ;continue;end
...
function eval_logpost(yprd::Array{Float64,1},mujoint::Array{Float64,1},Sjoint::Array{Float64,2})
    Sjinv,logdetSj = Simple_inv(Sjoint) 
    yjoint = yprd
...
function evalPH_t(Kernel,Ktt,Kpt,Kpp,tstep,mstep,ith,xt::T,yt::T,lt,Mysigma,muy,mstd,xprd::T,yprd::T,lp,xd::T,yd::T,ld,xd2::T,yd2::T,ld2,Theta,Monotonic,Convex,GPmode,fixR,prePH) where {T<:AbstractArray{Float64,1}}
    #time0 = time()
    ...
    if Monotonic==true ## Not complete
        ...
        KernelMat!(Ktt,Kernel,Theta,xt,xt,0,false,false) 
        Ktt += Mysigma
        
        t0_1 = time()
        
        KernelMat!(Kpp,Kernel,Theta,xprd,xprd,0,false,false)            
        
        t0_2 = time()
        
        KernelMat!(Kpt,Kernel,Theta,xprd,xt,0,false,false) 
        ...
        #mu_y = reshape([ 0.0 for i=1:lp],lp); mu_yt= reshape([ 0.0 for i=1:lt],lt)
        #println("typeof(cLinv) ",typeof(cLinv), " typeof(Kpt) ", typeof(Kpt))
        #mujoint, Sjoint  = calcSj(cLinv,Kpt,Kpp,yt)
    ...
    #term1 = -0.5*transpose(yt)*Kttinv*yt
    term1 = -0.5*transpose(yt)*(transpose(cLinv)*(cLinv*yt))
    term2 = -0.5*logdetK
    llh = term1 + term2 
    ...
    if tstep == 1
        ###########
        #der,der2 = eval_der(tstep,mstep,xt,yt,xprd,yprd,mstd,R,sigR,sigfac)
        der = -1.e+30
        der2 = -1.e+30
        ###########
...
function KernelMat!(K::Array{Float64,2},ktype,Theta,x,y,kind,pder=false,diag=false) 
